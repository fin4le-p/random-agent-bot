...
vs_group = app_commands.Group(name="vs", description="Riot連携機能")
...
@vs_group.command(name="status", description="Riot連携状況を確認します")
async def riot_status(interaction: discord.Interaction):
    if not INTERNAL_API_KEY:
    ...
    if status_code != 200:
        return await interaction.response.send_message(f"エラー: {status_code} {j}", ephemeral=True)

    if not j.get("linked"):
        return await interaction.response.send_message("❌ まだ連携されていません。/vs link から連携してね。", ephemeral=True)

    msg = "✅ 連携済みです"
    gn = j.get("riot_game_name") or ""
    tl = j.get("riot_tag_line") or ""
    if gn and tl:
        msg += f"\nRiot: `{gn}#{tl}`"
    if j.get("riot_puuid"):
        msg += f"\nPUUID: `{str(j.get('riot_puuid'))[:8]}...`"
    return await interaction.response.send_message(msg, ephemeral=True)


@vs_group.command(name="me", description="Riot疎通確認 + 直近試合（戦績）表示")
async def riot_me(interaction: discord.Interaction):
    if not INTERNAL_API_KEY:
        return await interaction.response.send_message("サーバー設定エラー: INTERNAL_API_KEY未設定", ephemeral=True)

    linked = await _ensure_linked_or_prompt(interaction)
    if not linked:
        return
